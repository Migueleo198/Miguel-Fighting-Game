...
class Sprite {
    ...
    constructor({ position, velocity, imageSrc, scale = 1, framesMax, offset, sprites, type, hitBox }) {
        ...
        this.type = type;
        this.off = false;
        this.oppositePositionHitBox=null;
        this.newPositionHitBox=null;
    ...
    update(gravity,opponent) {
        ...
        if (this.image === this.sprites.death.image && this.type == 'enemy' || this.image === this.sprites.deathOpposite.image && this.type == 'player') {
            this.keep = true;
        ...
        //OPPOSITE DIRECTIONS TO BASE DEATH FLAGS
        if(this.image == this.sprites.deathOpposite.image && this.framesCurrent>=6 && this.type==='enemy'){
            this.dead=true;
        }

        if(this.image == this.sprites.death.image && this.framesCurrent>=6 && this.type==='player'){
            this.dead=true;
        }
    ...
    switchSprite(sprite) {
        ...
        if (this.image == this.sprites.deathOpposite.image && this.framesCurrent < this.sprites.deathOpposite.framesMax - 1) { return }
        ...
        if (this.image == this.sprites.deathOpposite.image && this.type == 'player') { return };

        //OVERRIDE IDLE OPPOSITE ANIMATION FOR WHEN HIT FROM THE BACK CHANGE DIRECTION
        ...
        switch (sprite) {
            ...
            case 'death':
                if (this.image !== this.sprites.death.image) {
                    // Check if the last idle animation was the opposite one
                    if (this.image === this.sprites.idleOpposite.image) {
                        this.image = this.sprites.deathOpposite.image;
                        this.framesMax = this.sprites.deathOpposite.framesMax;
                    } else {
            ...
            case 'deathOpposite': if (this.image != this.sprites.deathOpposite.image) {
                this.image = this.sprites.deathOpposite.image;
                this.framesMax = this.sprites.deathOpposite.framesMax;
                this.framesCurrent = 0;
            } break;
